// src/app/status/page.tsx (NEW "LOGGED IN" LAYOUT)
"use client";
...
// --- COMPONENT to safely inject Viral Loops HTML ---
// This component is unchanged
const VlHtmlWidget: React.FC<{ htmlString: string }> = ({ htmlString }) => {
  useEffect(() => {
    // We must manually tell the VL script to "create" the widget
    // after we've injected its HTML.
...
// Helper component for the benefit lists
...
export default function MyStatusPage() {
  ...
  // We'll create a "First Name Last Name" from the email for the demo
          ...
          <>
            ...
            >
              ...
              <div 
                ...
                style={{ backgroundImage: `url('/my-mystatus-image.png')` }}
              >
                {/* This div acts as a 'fill' for the grid cell */}
                <div className="w-full h-full min-h-[400px] bg-cover bg-center" style={{ backgroundImage: `url('/my-mystatus-image.png')` }}></div>
              </div>
            </motion.div>
          </>
        )}
            ...
            <button
              onClick={() => {
                if (window.confirm("Are you sure you want to log out and clear your local status?")) {
                  clearUser(); 
                  router.refresh(); // Refresh to re-trigger the hook
                }
              }}
